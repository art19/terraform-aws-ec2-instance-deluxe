...
resource "aws_cloudwatch_metric_alarm" "this_instance_failed" {
  ...
  alarm_name          = "${var.instance_count > 1 || var.use_num_suffix ? format("%s${var.num_suffix_format}", var.name, count.index + 1) : var.name}-instance-status-check"
...
resource "aws_cloudwatch_metric_alarm" "this_system_failed" {
  ...
  alarm_name          = "${var.instance_count > 1 || var.use_num_suffix ? format("%s${var.num_suffix_format}", var.name, count.index + 1) : var.name}-system-status-check"
