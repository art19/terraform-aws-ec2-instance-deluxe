...
resource "aws_cloudwatch_metric_alarm" "this_instance_failed" {
  count = var.autoreboot_enabled == "true" ? var.instance_count : 0

  alarm_name          = "${var.instance_count > 1 || var.use_num_suffix ? format("%s-%s${var.num_suffix_format}", var.name, var.dns_name, count.index + 1) : var.name}-instance-status-check"
...
resource "aws_cloudwatch_metric_alarm" "this_system_failed" {
  count = var.autorecovery_enabled == "true" ? var.instance_count : 0

  alarm_name          = "${var.instance_count > 1 || var.use_num_suffix ? format("%s-%s${var.num_suffix_format}", var.name, var.dns_name, count.index + 1) : var.name}-system-status-check"
